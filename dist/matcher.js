 ...
 */
export async function expectVisual(page, options) {
    ...
    try {
        ...
        const result = await provider.analyze(screenshot, options.rules.map(description => ({ description })));
        for (const r of result.results) {
            if (!r.pass) {
                issues.push(`[视觉规则] "${options.rules[r.ruleIndex]}" 未通过: ${r.reason}`);
            }
        }
    }
    catch (error) {
    ...
    if (doA11y) {
        try {
            ...
            for (const v of a11yResult.violations) {
                issues.push(`[可访问性] [${v.impact}] ${v.id}: ${v.help} (${v.helpUrl})`);
            }
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
    ...
    if (issues.length > 0) {
        ...
        throw new Error(`视觉检查失败 (${issues.length} 个问题):\n${issues.join('\n')}`);
    }
}
/**
